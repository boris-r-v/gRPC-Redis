...
class ServerImpl{
    ...
    private:
        class CallData {
	    public:
	        // Take in the "service" instance (in this case representing an asynchronous server) and the completion queue "cq" used for asynchronous communication with the gRPC runtime.
	        CallData(bsan::BalanceRPC::AsyncService* service, grpc::ServerCompletionQueue* cq):
	            service_(service), cq_(cq), 
		        responder_(&ctx_), status_(CREATE) 
            {
                Proceed();       // Invoke the serving logic right away.
            }
	        ...
	        void Proceed() {
	            if (status_ == CREATE) {
                    ...
                    std::cout << "async_server: " << request_.name() << std::endl;
