	    ...
	    public:
	        ...
	        Task Proceed() override {
	            if (status_ == CREATE) {
                    ...
                    std::osyncstream(std::cout) << "proceed0 key:<" << key<< "> thread_id<"<< std::this_thread::get_id() <<"> \n";
                    try {
                        std::osyncstream(std::cout) << "proceed1 key:<" << key<< "> thread_id<"<< std::this_thread::get_id() <<"> \n";
                        auto data = co_await redis_ -> get( key );
                    ...
                    status_ = FINISH;
                    responder_.Finish(reply_, grpc::Status::OK, this);
                    std::osyncstream(std::cout) << "proceed3 key:<" << key<< "> thread_id<"<< std::this_thread::get_id() <<"> \n";
                } else if(status_ == FINISH) {
                    //GPR_ASSERT(status_ == FINISH);
                    delete this; 
                }else {
